...
## 0. 처음 시작하는 분을 위한 15단계 빠른 시작

이 구간은 전체 README의 **길잡이**다. 여기 있는 체크박스만 보고 명령을 건너뛰지 말고, 각 단계에 표시된 본문의 번호로 이동해 명령·정상 결과·오류 해결을 확인한다.

> **진행 원칙:** 한 번에 한 명령 블록만 실행한다. 각 단계의 완료 기준을 확인한 뒤 다음 단계로 이동한다. 오류가 발생하면 다음 단계로 넘어가지 않는다.

### 0.1 예상 소요 시간

| 구간 | 예상 시간 | 비고 |
|---|---:|---|
| 환경·저장소 준비 | 20~30분 | 패키지 다운로드 시간에 따라 달라짐 |
| 계정·권한·앱 배치 | 40~60분 | Linux 권한을 처음 다루면 더 걸릴 수 있음 |
| SSH·UFW | 30~50분 | 접속 중단을 막기 위해 천천히 진행 |
| 앱·monitor 검증 | 40~60분 | 정상·장애 상황을 각각 시험 |
| logrotate·cron | 30~50분 | cron 확인에 최소 70초 대기 포함 |
| 보너스·증빙 정리 | 40~60분 | 실제 출력은 개인별로 다름 |
| **합계** | **약 3~5시간** | 오류 해결과 학습 시간은 별도 |

처음이라면 한 번에 끝내려 하지 말고 `환경 준비 → 권한 설정 → 관제 검증 → 증빙 정리`의 네 구간으로 나누어 진행해도 된다.

### 0.2 전체 진도표

체크박스는 GitHub 화면에서 직접 바뀌지 않는다. 자신의 메모에서 체크하거나, 실제 실행 결과를 확인한 뒤 별도 커밋으로 갱신한다.

- [ ] **1. 실행 환경 확인** — 7단계: Ubuntu 24.04, 계정, 저장소 위치 확인
- [ ] **2. 필수 패키지 설치** — 7단계: `ss`, `sshd`, `ufw`, `cron`, `logrotate`, `getfacl` 확인
- [ ] **3. 계정·그룹 생성** — 8단계: 세 계정과 두 그룹 확인
- [ ] **4. 디렉터리·권한 설정** — 9~10단계: 실제 계정별 허용·차단 시험
- [ ] **5. 앱 압축 해제·배치** — 11단계: 앱 파일과 소유권 확인
- [ ] **6. SSH 설정** — 12단계: 문법 검사 후 `20022`, Root 차단 확인
- [ ] **7. UFW 설정** — 13단계: SSH 안전 확인 후 두 포트만 허용
- [ ] **8. 환경 변수·키 파일 설정** — 14~15단계: 값·경로·접근 권한 확인
- [ ] **9. 앱 실행** — 16~18단계: Boot Sequence 5개 `[OK]`, `Agent READY`, 포트 LISTEN 확인
- [ ] **10. monitor 직접 검증** — 19~20단계: 정상 종료 `0`, 로그 누적 확인
- [ ] **11. 장애 검증** — 20단계: 프로세스 없음·포트 닫힘에서 각각 종료 `1` 확인
- [ ] **12. logrotate 검증** — 21단계: `size 10M`, `rotate 10`, 드라이런 확인
- [ ] **13. cron 자동 실행 검증** — 22~23단계: 70초 전후 로그 줄 수 증가 확인
- [ ] **14. 보너스 검증** — 24~25단계: 통계 보고서와 `/tmp` 아카이브 시험
- [ ] **15. 제출 증빙 완성** — 26단계: 정적 검사 후 실제 출력을 `docs/command-log.md`에 기록

### 0.3 필수 과정과 선택 과정

| 구분 | 범위 | 완료 판단 |
|---|---|---|
| 필수 | 1~23단계, 26단계 실행 증빙 | SSH·UFW·권한·앱·monitor·로그·cron·logrotate가 실제로 동작 |
| 보너스 | 24단계 `report.sh`, 25단계 로그 아카이브 | 정상 결과와 예외 상황을 직접 실행하고 설명 가능 |
| 제출 준비 | `command-log.md`, 체크리스트, 질의응답 | 날짜·환경·커밋과 실제 출력이 있고 민감정보가 제거됨 |

README에 코드와 절차가 있다고 해서 실습이 완료된 것은 아니다. 이 저장소에서 **준비됨**은 구현·검증 방법이 있다는 뜻이고, **검증 완료**는 자신의 Ubuntu에서 직접 실행한 출력이 있다는 뜻이다.

### 0.4 위험 작업의 중단 기준

다음 조건에서는 명령을 계속 실행하지 말고 현재 터미널을 유지한 채 해당 단계의 오류 해결을 확인한다.

- `sshd -t`가 실패하면 SSH 서비스를 재시작하지 않는다.
- 새 SSH 포트 `20022` 접속을 확인하기 전에는 기존 SSH 창을 닫지 않는다.
- UFW에 `20022/tcp` 허용이 보이기 전에는 방화벽을 활성화하지 않는다.
- `whoami`가 예상 계정과 다르면 권한·앱 실행 명령을 진행하지 않는다.
- `Agent READY` 또는 `15034 LISTEN`이 없으면 monitor와 cron 단계로 넘어가지 않는다.
- 실제 비밀번호, API 키, 토큰, 개인 IP가 출력에 포함되면 Git에 추가하지 않는다.

### 0.5 100점 준비 상태를 판단하는 네 가지 질문

아래 네 질문에 모두 “예”라고 답할 수 있을 때 제출 완성도와 재현성이 높다.

- [ ] **구현:** 필수 기능과 보너스 스크립트가 기본 브랜치에 있는가?
- [ ] **실행:** 자신의 Ubuntu에서 정상·장애 상황을 직접 재현했는가?
- [ ] **증빙:** `docs/command-log.md`에 날짜·환경·커밋·실제 출력이 있는가?
- [ ] **설명:** 28단계 질문을 AI 답변이 아닌 자신의 말로 설명할 수 있는가?

`docs/command-log.md`의 `TODO`나 `미실행`은 실제 검증을 마친 뒤에만 교체한다. 참고용 출력이나 다른 사람의 실행 결과를 자신의 증빙으로 사용하지 않는다.
