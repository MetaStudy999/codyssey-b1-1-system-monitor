...
1. 앱을 root로 실행하면 앱 버그나 침해가 발생했을 때 시스템 전체가 위험해진다. `agent-admin` 같은 일반 계정으로 실행하면 피해 범위를 해당 계정 권한으로 제한할 수 있다.
2. `agent-common`은 업로드 디렉터리처럼 세 계정이 함께 접근할 리소스용이고, `agent-core`는 API 키와 로그처럼 운영 핵심 리소스용이다. 역할별 그룹을 나누면 최소 권한 원칙을 적용하기 쉽다.
3. `upload_files`는 테스트 계정까지 함께 접근할 수 있어야 하지만, `api_keys`는 민감 파일이므로 `agent-admin`, `agent-dev`만 접근해야 한다. 그래서 그룹을 다르게 둔다.
4. 프로세스가 살아 있어도 포트를 열지 못하면 서비스 요청을 받을 수 없다. 반대로 포트가 열려 있어도 의도한 앱이 아닐 수 있으므로 프로세스와 포트를 모두 확인한다.
5. 방화벽 비활성은 보안 위험이지만 앱 동작 자체가 즉시 중단된 것은 아니므로 WARNING이다. 포트가 LISTEN이 아니면 서비스 요청을 받을 수 없으므로 Health Check 실패로 보고 `exit 1`이다.
6. `>`는 기존 파일을 덮어쓰고, `>>`는 뒤에 누적한다. 관제 로그는 시간순 이력이 중요하므로 `>>`를 써야 한다.
7. cron은 화면에 직접 오류를 보여주지 않는다. `cron.log`에 stdout/stderr를 남기면 자동 실행 실패 원인을 나중에 추적할 수 있다.
8. logrotate는 크기와 보관 개수 기준으로 현재 로그를 관리하고, 시간 기반 아카이브는 오래된 로그를 별도 위치에 압축 보관한다. 두 방식을 함께 쓰면 디스크 용량과 장기 보존을 모두 관리할 수 있다.
9. `report.sh`는 분석 가능한 샘플 수가 `0`이면 평균 계산을 하지 않고 오류 메시지와 함께 종료한다. 그래서 `sum / count`에서 `count`가 0이 되는 상황을 피한다.
10. 먼저 `ps`로 앱 프로세스 존재 여부를 확인하고, `ss`로 `15034` LISTEN 상태를 확인한다. 그다음 `monitor.log`에서 최근 관제 상태를 보고, cron 자동 실행 문제라면 `cron.log`에서 stderr와 실행 기록을 확인한다.
